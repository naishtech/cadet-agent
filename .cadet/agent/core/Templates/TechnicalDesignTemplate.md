# Technical Design Template

Purpose: Translate approved requirements into an implementation-ready design with explicit red/green TDD strategy and traceability to execution.

## Backlinks
- Identity: [Identity](../Identity.md)
- Learner model: [LearnerModel](../LearnerModel.md)
- Principles: [Principles](../Principles.md)
- Workflow: [Workflow](../Workflow.md)
- Requirements document: [TODO]
- Related project plan: [TODO]

## Document Control
- Project: [TODO]
- Feature or Change Name: [TODO]
- Author: [TODO]
- Status: Draft | In Review | Final
- Version: [TODO]
- Last Updated: [TODO]

## Learner Context
- Relevant learner dimension: [TODO]
- Assumed learner tier: [TODO]
- Operating mode during design review: Instruction-first | Implementation-first | Hybrid
- Explanation depth or handoff notes: [TODO]

## Design Summary
- Overview: [TODO]
- Objectives: [TODO]
- Scope Boundaries: [TODO]
- Success Metrics: [TODO]

## Requirements Mapping
| Requirement ID | Design Section | Notes |
|---|---|---|
| [TODO] | [TODO] | [TODO] |

## Current State
- Existing Architecture: [TODO]
- Technical Constraints: [TODO]
- Pain Points: [TODO]

## Proposed Design
- Component Changes: [TODO]
- Data Flow: [TODO]
- Interfaces and Contracts: [TODO]
- Error Handling Strategy: [TODO]
- Security Considerations: [TODO]
- Performance Considerations: [TODO]

## Technology Choices
- Proposed Technology: [TODO]
- Long-Term Support Evaluation: [TODO]
- Community and Support Evaluation: [TODO]
- User Familiarity and Consent Notes: [TODO]
- Alternatives Considered: [TODO]

## TDD Strategy (Red/Green)
### Test Matrix
| Acceptance Criteria ID | Failing Test (Red) | Implementation Step (Green) | Regression Coverage |
|---|---|---|---|
| [TODO] | [TODO] | [TODO] | [TODO] |

### Test Execution Plan
- Unit Tests: [TODO]
- Integration Tests: [TODO]
- End-to-End Checks: [TODO]

## Rollout and Migration
- Rollout Plan: [TODO]
- Migration Steps: [TODO]
- Fallback or Rollback Plan: [TODO]

## Traceability to Planning
- Planned project phases: [TODO]
- Linked epic IDs: [TODO]
- Initial task decomposition notes: [TODO]

## Review and Approval
- Reviewer(s): [TODO]
- Review Notes: [TODO]
- **Multi-model review:** Recommend the user optionally copy this design into a separate chat with a different AI model for an independent review. A fresh context window and different model perspective help catch blind spots.
- Approval Decision: [TODO]
- Approval Date: [TODO]

## Change History
| Date | Version | Change Summary | Reason | Impacted Requirements | Updated By |
|---|---|---|---|---|---|
| [TODO] | [TODO] | [TODO] | [TODO] | [TODO] | [TODO] |

---
