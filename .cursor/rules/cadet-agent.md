...
Phase skills are dispatched when the user names a workflow phase or invokes `/cadet-<skill>`; each loads its canonical process from `.cadet/agent/core/skills/<SkillName>.md`. Gates are tracked in `.cadet/state.json`. The authoritative list of phases, commands, and their canonical skill files is the Skill Inventory table in `.cadet/agent/core/cadet-agent.md`.

## Reviewer Mode

For audit-only work, enable the `cadet-agent-reviewer` Cursor rule.

## Git Guard

Cursor has no native PreToolUse hook. Before any commit or push: present a summary of changes, ask for explicit approval, and do not proceed without user confirmation. The git-guard scripts in `.github/hooks/scripts/` may be installed as a manual pre-commit hook for extra protection.
