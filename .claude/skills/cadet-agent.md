---
name: cadet-agent
description: Cadet-Agent operating rules for Unity and game-development workflows. Use when working on Unity projects, game development, or any repository that has a .cadet directory.
---

- Read `.cadet/agent/core/cadet-agent.md` as the single condensed instruction file (all non-negotiable rules, workflow routing, hard-gate protocol, state management, skill dispatch, Unity-specific rules, document rules, Git workflow, framework sync, context management). Condensed from 16 original `docs/core/` files as of v0.5.0.
- When dispatching a workflow phase, read the canonical skill from `.cadet/agent/core/skills/<SkillName>.md` so it becomes the primary instruction context.
- Apply the operational files: `.cadet/agent/core/GitFirstRule.md`, `.cadet/agent/core/FrameworkSyncGate.md`, `.cadet/agent/core/KickoffFlow.md`, and `.cadet/agent/core/FirstResponseFormat.md`.
- Full rationale, examples, anti-patterns, and detailed reference live in `docs/` (skills, guidance, standards, templates).
- Repository-specific policy overlays live in `.cadet/agent/policies`.
- Planning artifacts live in `.cadet/agent/project-plans` unless an active policy says otherwise.
- Use TDD when testing is valid, reproduce defects before fixing them, and keep regression coverage.
