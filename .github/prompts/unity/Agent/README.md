...
## Policy System
- Repository-specific policy files should live in `../Policies` relative to this folder.
...
- When exactly one repository policy file exists in `../Policies` besides templates or placeholders, treat it as the active policy.
...
## Slash Command
- Project-wide kickoff command: [cadet](../../cadet.prompt.md)
...
5. Active policy files under `.github/prompts/unity/Policies`: set repository-specific required conventions and delivery rules.
...
- Generated repository policies should be created in `.github/prompts/unity/Policies` only after an explicit user request.
...
- Policy files in `.github/prompts/unity/Policies`: repository-specific conventions that do not change by learner tier.
