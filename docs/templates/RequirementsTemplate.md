# Requirements Template

Purpose: Capture clear, testable requirements with Given/When/Then acceptance criteria that drive design, planning, and implementation.

## Backlinks
- Identity: [Identity](../core/Identity.md)
- Learner model: [LearnerModel](../core/LearnerModel.md)
- Principles: [Principles](../core/Principles.md)
- Workflow: [Workflow](../core/Workflow.md)
- Related technical design: [TODO]
- Related project plan: [TODO]

## Document Control
- Project: [TODO]
- Feature or Change Name: [TODO]
- Author: [TODO]
- Status: Draft | In Review | Final
- Version: [TODO]
- Last Updated: [TODO]

## Learner Context
- Relevant learner dimension: [TODO]
- Assumed learner tier: [TODO]
- Operating mode: Instruction-first | Implementation-first | Hybrid
- Explanation depth notes: [TODO]

## Context
- Problem Statement: [TODO]
- User Outcome: [TODO]
- Business or Gameplay Value: [TODO]
- In Scope: [TODO]
- Out of Scope: [TODO]

## Constraints and Assumptions
- Constraints: [TODO]
- Assumptions: [TODO]
- Dependencies: [TODO]
- Risks: [TODO]

## Acceptance Criteria (Given/When/Then)
### AC-01: [TODO]
- Given: [TODO]
- When: [TODO]
- Then: [TODO]
- Testability Notes: [TODO]

### AC-02: [TODO]
- Given: [TODO]
- When: [TODO]
- Then: [TODO]
- Testability Notes: [TODO]

### AC-03: [TODO]
- Given: [TODO]
- When: [TODO]
- Then: [TODO]
- Testability Notes: [TODO]

## Non-Functional Requirements
- Performance: [TODO]
- Security: [TODO]
- Reliability: [TODO]
- Usability: [TODO]

## Traceability
- Maps to technical design sections: [TODO]
- Maps to project plan phases: [TODO]
- Maps to epic IDs: [TODO]

## Review and Approval
- Reviewer(s): [TODO]
- Review Notes: [TODO]
- Approval Decision: [TODO]
- Approval Date: [TODO]

## Change History
| Date | Version | Change Summary | Reason | Impacted AC IDs | Updated By |
|---|---|---|---|---|---|
| [TODO] | [TODO] | [TODO] | [TODO] | [TODO] | [TODO] |

---
